...
async function analyzePatterns() {
  ...
  errors.forEach(error => {
    ...
    commandErrors.set(error.command, (commandErrors.get(error.command) || 0) + 1);
  ...
  if (sortedCommands[0] && sortedCommands[0][1] > errors.length * 0.3) {
    logger.warn(`- The '${sortedCommands[0][0]}' command accounts for over 30% of errors. Consider reviewing its implementation.`);
  }
  
  const userErrors = errors.filter(e => e.category === 'USER_ERROR').length;
  if (userErrors > errors.length * 0.5) {
    logger.warn('- Over 50% of errors are user errors. Consider improving documentation or error messages.');
  }
  
  const criticalErrors = errors.filter(e => e.severity === 'CRITICAL').length;
  if (criticalErrors > 0) {
    logger.error(`- Found ${criticalErrors} critical errors that need immediate attention.`);
  }
}
