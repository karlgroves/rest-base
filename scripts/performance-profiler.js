...
class PerformanceMonitor {
  ...
  calculateAverageMemory() {
    ...
    const total = this.memorySnapshots.reduce((acc, snapshot) => ({
  ...
  calculatePeakMemory() {
    ...
    return this.memorySnapshots.reduce((peak, snapshot) => ({
  ...
  async exportReport(filePath = null) {
    ...
    const outputPath = filePath || this.options.outputFile || 
...
class CPUProfiler {
  ...
  getReport() {
    ...
    const totalCPU = this.samples.reduce((acc, sample) => ({
    ...
    return {
      ...
      summary: {
        ...
        avgCPU,
        peakMemory: this.samples.reduce((peak, sample) => 
          Math.max(peak, sample.memoryUsage.heapUsed), 0
        )
      }
    };
  }
...
class RequestAnalyzer {
  ...
  getReport() {
    ...
    const avgDuration = this.requests.reduce((sum, req) => sum + req.duration, 0) / this.requests.length;
...
class ReportGenerator {
  ...
  static generateConsoleReport(data) {
    ...
    if (data.summary) {
      console.log('\n' + color.cyan('Summary:'));
      console.log(`  Total Duration: ${color.yellow(this.formatDuration(data.summary.totalDuration))}`);
      ...
      if (data.summary.avgMemory) {
        console.log(`  Avg Memory: ${color.yellow(this.formatBytes(data.summary.avgMemory.heapUsed))}`);
      }
      
      if (data.summary.peakMemory) {
        console.log(`  Peak Memory: ${color.yellow(this.formatBytes(data.summary.peakMemory.heapUsed))}`);
      }
    }
    ...
    if (data.metrics && data.metrics.length > 0) {
      ...
      sorted.forEach((metric, i) => {
        console.log(`  ${i + 1}. ${color.yellow(metric.name)}: ${this.formatDuration(metric.duration)}`);
      });
    }
...
// Analyze command
async function analyzeCommand(logFile, options) {
  try {
    ...
    if (report.memorySnapshots) {
      ...
      const isIncreasing = memoryTrend.every((snapshot, i) => 
        i === 0 || snapshot.memory.heapUsed >= memoryTrend[i - 1].memory.heapUsed
      );
...
async function benchmarkCommand(options) {
  ...
  console.log(`${color.cyan('Synchronous Operations:')} ${ReportGenerator.formatDuration(report.sync.avg)} avg`);
  console.log(`${color.cyan('Asynchronous Operations:')} ${ReportGenerator.formatDuration(report.async.avg)} avg`);
  console.log(`${color.cyan('Memory Allocation:')} ${ReportGenerator.formatBytes(report.memory.avg)} avg`);
